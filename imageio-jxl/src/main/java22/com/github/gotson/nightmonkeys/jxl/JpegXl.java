            ...
            try (var arena = Arena.ofConfined()) {
                ...
                while (true) {
                    ...
                    if (status == JxlDecoderStatus.JXL_DEC_BASIC_INFO) {
                        ...
                        var iccSize = arena.allocate(C_LONG);
                        int iccSizeResult;
                        if (legacyIcc()) {
                            iccSizeResult = decode_h.JxlDecoderGetICCProfileSizeLegacy(dec, MemorySegment.NULL, JxlColorProfileTarget.JXL_COLOR_PROFILE_TARGET_DATA.intValue(), iccSize);
                        } else {
                        ...
                        if (legacyIcc()) {
                            iccProfileResult = decode_h.JxlDecoderGetColorAsICCProfileLegacy(dec, MemorySegment.NULL, JxlColorProfileTarget.JXL_COLOR_PROFILE_TARGET_DATA.intValue(),
                ...
                return new BasicInfo(
                    JxlBasicInfo.xsize(info),
                    JxlBasicInfo.ysize(info),
                    JxlBasicInfo.alpha_bits(info) > 0,
                    JxlBasicInfo.have_animation(info) > 0,
                    JxlBasicInfo.num_color_channels(info),
                    icc,
    ...
    public static void decode(final ImageInputStream stream, final BasicInfo info, WritableRaster raster, ImageReadParam param, int imageIndex) throws JxlException {
        ...
        try {
            ...
            try (var arena = Arena.ofConfined()) {
                ...
                JxlPixelFormat.num_channels(format, 4);
                JxlPixelFormat.data_type(format, dataType.intValue());
                JxlPixelFormat.endianness(format, JxlEndianness.JXL_BIG_ENDIAN.intValue());
                JxlPixelFormat.align(format, 0);
                ...
                while (true) {
                    ...
                    if (status == JxlDecoderStatus.JXL_DEC_NEED_IMAGE_OUT_BUFFER) {
                        var bufferSizeBytes = arena.allocate(C_LONG);
                        ...
                        var bufferSizeExpected = info.width() * info.height() * JxlPixelFormat.num_channels(format) * dataType.sizeBytes();
                        ...
                        pixels = arena.allocate(C_CHAR, bufferSizeBytesValue);
                        if (JxlDecoderStatus.JXL_DEC_SUCCESS !=

            ...
            try (ResourceScope scope = ResourceScope.newSharedScope()) {
                ...
                while (true) {
                    ...
                    if (status == JxlDecoderStatus.JXL_DEC_NEED_IMAGE_OUT_BUFFER) {
                        var bufferSizeBytes = SegmentAllocator.nativeAllocator(scope).allocate(C_LONG, 0);
                        ...
                        if (JxlDecoderStatus.JXL_DEC_SUCCESS !=
                            JxlDecoderStatus.fromId(decode_h.JxlDecoderSetImageOutBuffer(dec, format, MemorySegment.ofByteBuffer(pixels), bufferSizeBytes.get(C_LONG, 0)))) {

...
public class WebP {
    ...
    public static BasicInfo getBasicInfo(final ImageInputStream stream) throws WebpException {
        try (var arena = Arena.ofConfined()) {
            ...
            WebPData.bytes$set(demuxData, data);
            ...
            if (!demux.equals(MemorySegment.NULL)) {
                ...
                if (flags.contains(WebPFeatureFlags.ICCP_FLAG)) {
                    demux_h.WebPDemuxGetChunk(demux, arena.allocateUtf8String("ICCP"), 1, chunkIter);
                    ...
                    WebPData.bytes$get(chunkWebpData).asSlice(0, size).asByteBuffer().get(iccData);
    ...
    public static void decodeVP8(final ImageInputStream stream, BasicInfo info, final WritableRaster raster, ImageReadParam param) throws WebpException {
        try (var arena = Arena.ofConfined()) {
            ...
            WebPRGBABuffer.rgba$get(rgba).asSlice(0, (long) raster.getWidth() * raster.getHeight() * 4).asByteBuffer().asIntBuffer().get(pixelsArray);
